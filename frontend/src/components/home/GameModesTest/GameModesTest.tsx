...
function GameModesTest() {
    ...
    return (
        <div id="games-and-points"  className={`grid grid-cols-2 ml-[9vw]  mr-[9vw] gap-[20.91vw] mt-[5.64vh]`}>

            <section ref={ref} id="game-mode-section" className="">
                <h1 className={`font-extrabold text-[${scale.width}px] mb-[${scale.mb}px] font-sans`}> Game Modes </h1>
                ...
                <div id="play-card" className={`w-[${scale.play_size}px]`}>
                    <CornerLinedCardTest childComp={
                        <h2 className={`play font-Rowdies text-[${scale.font_size}px]`}> PLAY </h2>
                    } fill="[color:#FE5821]" cornerredius="3" stroke="[color:#FE5821]"  cornershape={[24,0,24,0]} strokesize={0} width={200} height={87} margine={`mt-[${scale.play_button_margin3}px]`} ratio={43/100} />
                </div>
            </section>
            <section style={{'borderColor':"black"}} id="points-section" className=" grid grid-cols-1   ">
                <PointSection/>
            </section>
        </div>
    )
