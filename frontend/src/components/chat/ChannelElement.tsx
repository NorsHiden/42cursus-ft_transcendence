...
const leaveChannel = (channelId:number) => {
    ...
    toast.promise(
        axios.delete(`/api/channels/${channelId}/leave`),
        {
...
const ChannelElement: React.FC<ChannelProps> = ({ name, avatar, role,ChanelId }) => {
    ...
    const {channels,setChannels,setShowUpdateChannelModal,setSelectedChannel} = useSelectedChannel();
    ...
    <>
      ...
      <div onClick={(e) =>{
        ...
        <ContextMenu
          menuItems={role === 'owner' ? OwnerMenuItems : MemberMenuItems}
        />
      </div>
    </>
  );
